...
class BuyOrder:
    def __init__(self, symbol, current_price, total_amount, weight, exclude_count):
        self.symbol = symbol
    ...
    def distribute_orders(self, drop_percentage, drop_count, distribution_type, confidence=1.0):
        if self.DistributionType.LINEAR == distribution_type:
            self.linear_distribution(drop_percentage, drop_count, 16777216)
        elif self.DistributionType.LOG_LINEAR_II == distribution_type:
            self.log_linear_distribution(drop_percentage, drop_count, 3)
        elif self.DistributionType.LOG_LINEAR_I == distribution_type:
            self.log_linear_distribution(drop_percentage, drop_count, 2)
        elif self.DistributionType.PARABOLIC_II == distribution_type:
            self.parabolic_distribution2(drop_percentage, drop_count)
        elif self.DistributionType.PARABOLIC_I == distribution_type:
            self.parabolic_distribution(drop_percentage, drop_count)
        elif self.DistributionType.EXPONENTIAL == distribution_type:
            self.exponential_distribution(drop_percentage, drop_count, 1.2)
        elif self.DistributionType.FIBONACCI == distribution_type:
            self.fibonacci_distribution(drop_percentage, drop_count)

    def linear_distribution(self, drop_percentage, drop_count, difference):
        total_weight = 0
        ...
        for n in range(1, drop_count + 1 - self.exclude_count):
            ...
            price = UpbitTickSystem.round_down(UpbitTickSystem.round_up(self.current_price), 
                                             (n - 1) * (drop_percentage * height_weight))
            quantity = self.total_amount * n / total_weight
            self.place_buy_order(price, quantity)

    def log_linear_distribution(self, drop_percentage, drop_count, weight):
        total_weight = 0
        for n in range(1, drop_count + 1):
    ...
    def parabolic_distribution(self, drop_percentage, drop_count):
    ...
    def parabolic_distribution2(self, drop_percentage, drop_count):
    ...
    def exponential_distribution(self, drop_percentage, drop_count, exponent):
        h = drop_count
        r = exponent
    ...
    def fibonacci_distribution(self, drop_percentage, drop_count):
    ...
    def place_buy_order(self, price, quantity):
        ...
        if 'uuid' in response_dict:
            ...
            print_log(LogLevel.INFO, f"Buy order placed at {price:,.2f} KRW, amount: {quantity:,.0f} KRW")
        else:
...
class SymbolSelector:
    ...
    def select_best_symbol():
        ...
        if not valid_symbols:
            print_log(LogLevel.WARNING, "No valid symbols found with sufficient trading volume")
            return None
...
if __name__=="__main__":
    try:
        ...
        drop_percentage = args.drop_percentage if args.drop_percentage else 0.25
        distribution_type = args.distribution_type if args.distribution_type else BuyOrder.DistributionType.LOG_LINEAR_II
        distribution_weight = args.weight if args.weight else 0.032
        ...
        while True:
                    ...
                    with open("../log/command.txt", 'r', encoding='utf-8') as f:
                        ...
                        for line in lines:
                            ...
                            if parts[0] == 'SYMBOL':
                                ...
                                confidence = analyzer.calculate_composite_confidence()
                            if parts[0] == 'RATE':
                                profit_percentage = float(parts[1])
                            if parts[0] == 'EXIT':
            ...
            if trading_manager.should_place_buy_orders():
                ...
                buyer = BuyOrder(symbol, analyzer.candle.current_price, S, distribution_weight, 0)
                buyer.distribute_orders(drop_percentage, drop_count, distribution_type, confidence)
            ...
            while True:
                ...
                # 매수 주문 타임아웃 체크 (1분)
                if trading_manager.is_buy_timeout():
                    print_log(LogLevel.WARNING, f"Buy order timeout after {trading_manager.buy_timeout_seconds} seconds - cancelling all buy orders")
                    OrderCanceler().cancel_buy_orders()
                    break

                if trading_manager.should_wait_for_buy_execution():
                    balance, locked, avg_buy_price = AccountChecker().get_symbol_info(symbol)
                    if balance + locked > 0.00001:
                        trading_manager.mark_buy_orders_executed()
                        print_log(LogLevel.SUCCESS, "Buy orders executed - Starting sell process")

                if trading_manager.buy_orders_executed:
                    is_trading_complete = sell_controller.manage_sell_orders(
                        symbol, profit_targets, trading_manager, wait_count
                    )
            ...
            print_log(LogLevel.INFO,
                     f"Profit: {int(S - InitialBalance):+,} ({datetime.now() - START_TIME})")
            
            log_balance(S)
            S = int(S * COMMISSION)
            
            trading_manager.reset()
            print_log(LogLevel.INFO, "Waiting for next cycle...")
